 ...
 *
 * Ported to Bluegene by Jan Stoess, IBM Corporation
 *        changed initialization to boot from fdt rather than PCI
 *        changed byte _and_ bit endianess to big endian
 *        added hardware header of 16 bytes
 * 
...
/* not tunable */
#define REAL_RX_BUF_SIZE (RX_BUF_SIZE + 14 + EXT_HWHDR_SIZE)	/* rx/tx mac addr + type */

#define MIN_TX_DESC_FREE	8

#define EXT_HWHDR_SIZE          16
/* register defines */
...
#define CFG_EXT_HWHDR	0x08000000
...
#define desc_addr_set(desc, addr)				\
	do {							\
		((desc)[0] = cpu_to_be32(addr));		\
		if (HW_ADDR_LEN == 8)		 		\
			(desc)[1] = cpu_to_be32(((u64)addr) >> 32);	\
	} while(0)
#define desc_addr_get(desc)					\
	(be32_to_cpu((desc)[0]) | \
	(HW_ADDR_LEN == 8 ? ((dma_addr_t)be32_to_cpu((desc)[1]))<<32 : 0))
...
#define __kick_rx(dev)	writel_be(CR_RXE, dev->base + CR)
...
static inline void kick_rx(struct net_device *ndev)
{
	...
	if (test_and_clear_bit(0, &dev->rx_info.idle)) {
		...
		writel_be(dev->rx_info.phy_descs +
 ...
 * possible.
 */
static inline void build_rx_desc(struct ns83820 *dev, __be32 *desc, dma_addr_t link, dma_addr_t buf, u32 cmdsts, u32 extsts)
{
	desc_addr_set(desc + DESC_LINK, link);
	desc_addr_set(desc + DESC_BUFPTR, buf);
	desc[DESC_EXTSTS] = cpu_to_be32(extsts);
	mb();
	desc[DESC_CMDSTS] = cpu_to_be32(cmdsts);
}
...
static inline int ns83820_add_rx_skb(struct ns83820 *dev, struct sk_buff *skb)
{
...
#if 0
	printk("next_empty[%d] nr_used[%d] next_rx[%d]\n",
	...
	buf = dma_map_single(/* dev->of_dev->dev */ NULL, skb->data,
			     REAL_RX_BUF_SIZE, PCI_DMA_FROMDEVICE);
	build_rx_desc(dev, sg, 0, buf, cmdsts, 0);
	/* update link of previous rx */
	if (likely(next_empty != dev->rx_info.next_rx))
		dev->rx_info.descs[((NR_RX_DESC + next_empty - 1) % NR_RX_DESC) * DESC_SIZE] = cpu_to_be32(dev->rx_info.phy_descs + (next_empty * DESC_SIZE * 4));
...
static void fastcall phy_intr(struct net_device *ndev)
{
	...
	cfg = readl_be(dev->base + CFG) ^ SPDSTS_POLARITY;

	if (dev->CFG_cache & CFG_TBI_EN) {
		/* we have an optical transceiver */
		tbisr = readl_be(dev->base + TBISR);
		tanar = readl_be(dev->base + TANAR);
		tanlpar = readl_be(dev->base + TANLPAR);
		...
		if ( (fullduplex = (tanlpar & TANAR_FULL_DUP)
			...
			writel_be(readl_be(dev->base + TXCFG)
			       | TXCFG_CSI | TXCFG_HBI | TXCFG_ATP,
			       dev->base + TXCFG);
			writel_be(readl_be(dev->base + RXCFG) | RXCFG_RX_FD,
			       dev->base + RXCFG);
			/* Light up full duplex LED */
			writel_be(readl_be(dev->base + GPIOR) | GPIOR_GP1_OUT,
			       dev->base + GPIOR);
		...
		if ((cfg & CFG_LNKSTS) &&
			...
			writel_be(new_cfg, dev->base + CFG);
...
static int ns83820_setup_rx(struct net_device *ndev)
{
	...
	writel_be(0, dev->base + RXDP_HI);
	writel_be(dev->rx_info.phy_descs, dev->base + RXDP);
	...
	if (!ret) {
		...
		writel_be(0x0001, dev->base + CCSR);
		writel_be(0, dev->base + RFCR);
		writel_be(0x7fc00000, dev->base + RFCR);
		writel_be(0xffc00000, dev->base + RFCR);
		...
		writel_be(dev->IMR_cache, dev->base + IMR);
		writel_be(1, dev->base + IER);
...
static void fastcall rx_irq(struct net_device *ndev)
{
	...
	while ((CMDSTS_OWN & (cmdsts = be32_to_cpu(desc[DESC_CMDSTS]))) &&
	       (cmdsts != CMDSTS_OWN)) {
		struct sk_buff *skb;
		u32 extsts = be32_to_cpu(desc[DESC_EXTSTS]);
		
                dma_addr_t bufptr = desc_addr_get(desc + DESC_BUFPTR);
                dprintk("cmdsts: %08x\n", cmdsts);
		dprintk("link: %08x\n", cpu_to_be32(desc[DESC_LINK]));
		dprintk("extsts: %08x\n", extsts);
		...
		dma_unmap_single(/* dev->of_dev->dev */ NULL, bufptr, RX_BUF_SIZE, PCI_DMA_FROMDEVICE);
...
#endif
			skb_put(skb, len);
                        if (dev->CFG_cache & CFG_EXT_HWHDR)
                            skb_pull(skb, EXT_HWHDR_SIZE);
...
static void do_tx_done(struct net_device *ndev)
{
	...
	dprintk("tx_done_idx=%d free_idx=%d cmdsts=%08x\n",
		tx_done_idx, dev->tx_free_idx, be32_to_cpu(desc[DESC_CMDSTS]));
	while ((tx_done_idx != dev->tx_free_idx) &&
	       !(CMDSTS_OWN & (cmdsts = be32_to_cpu(desc[DESC_CMDSTS]))) ) {
		struct sk_buff *skb;
		unsigned len;
		dma_addr_t addr;
		...
		if (skb) {
                        dma_unmap_single(/* dev->of_dev->dev */ NULL,
                                         addr,
                                         len,
                                         PCI_DMA_TODEVICE);
			...
			dma_unmap_page(/* dev->of_dev->dev */ NULL,
					addr,
					len,
					PCI_DMA_TODEVICE);
		...
		desc[DESC_CMDSTS] = cpu_to_be32(0);
		mb();
		desc = dev->tx_descs + (tx_done_idx * DESC_SIZE);
	}
...
static void ns83820_cleanup_tx(struct ns83820 *dev)
{
	...
	for (i=0; i<NR_TX_DESC; i++) {
		...
		dev->tx_skbs[i] = NULL;
		if (skb) {
                    __be32 *desc = dev->tx_descs + (i * DESC_SIZE);
                    dma_unmap_single(/* dev->of_dev->dev */ NULL,
                                     desc_addr_get(desc + DESC_BUFPTR),
                                     be32_to_cpu(desc[DESC_CMDSTS]) & CMDSTS_LEN_MASK,
                                     PCI_DMA_TODEVICE);
                    dev_kfree_skb_irq(skb);
                    atomic_dec(&dev->nr_tx_skbs);
		}
...
again:
...
#endif
        if (dev->CFG_cache & CFG_EXT_HWHDR)
            skb_push(skb, EXT_HWHDR_SIZE);


	len = skb->len;
	if (nr_frags)
		len -= skb->data_len;
	buf = dma_map_single(/* dev->of_dev->dev */ NULL, skb->data, len, PCI_DMA_TODEVICE);

	first_desc = dev->tx_descs + (free_idx * DESC_SIZE);

	for (;;) {
		volatile __be32 *desc = dev->tx_descs + (free_idx * DESC_SIZE);
                
                dprintk("frag[%3u]: %4u @ 0x%08Lx\n", free_idx, len,
			(unsigned long long)buf);
		last_idx = free_idx;
		free_idx = (free_idx + 1) % NR_TX_DESC;
		desc[DESC_LINK] = cpu_to_be32(dev->tx_phy_descs + (free_idx * DESC_SIZE * 4));
		desc_addr_set(desc + DESC_BUFPTR, buf);
		desc[DESC_EXTSTS] = cpu_to_be32(extsts);
		...
		buf = dma_map_page(/* dev->of_dev->dev */ NULL, frag->page,
				   frag->page_offset,
	...
	first_desc[DESC_CMDSTS] |= cpu_to_be32(CMDSTS_OWN);
...
static void ns83820_update_stats(struct ns83820 *dev)
{
	...
	dev->stats.rx_errors		+= readl_be(base + 0x60) & 0xffff;
	dev->stats.rx_crc_errors	+= readl_be(base + 0x64) & 0xffff;
	dev->stats.rx_missed_errors	+= readl_be(base + 0x68) & 0xffff;
	dev->stats.rx_frame_errors	+= readl_be(base + 0x6c) & 0xffff;
	/*dev->stats.rx_symbol_errors +=*/ readl_be(base + 0x70);
	dev->stats.rx_length_errors	+= readl_be(base + 0x74) & 0xffff;
	dev->stats.rx_length_errors	+= readl_be(base + 0x78) & 0xffff;
	/*dev->stats.rx_badopcode_errors += */ readl_be(base + 0x7c);
	/*dev->stats.rx_pause_count += */  readl_be(base + 0x80);
	/*dev->stats.tx_pause_count += */  readl_be(base + 0x84);
	dev->stats.tx_carrier_errors	+= readl_be(base + 0x88) & 0xff;
}
...
{
	struct ns83820 *dev = PRIV(ndev);
	u32 cfg = readl_be(dev->base + CFG) ^ SPDSTS_POLARITY;
...
static void ns83820_do_isr(struct net_device *ndev, u32 isr)
{
	...
	if ((ISR_RXRCMP & isr) && dev->rx_info.up)
		writel_be(CR_RXE, dev->base + CR);

	if (ISR_TXIDLE & isr) {
		u32 txdp;
		txdp = readl_be(dev->base + TXDP);
		dprintk("txdp: %08x\n", txdp);
	...
	if (dev->ihr)
		writel_be(dev->ihr, dev->base + IHR);
#endif
}
...
static void ns83820_do_reset(struct ns83820 *dev, u32 which)
{
	...
	writel_be(which, dev->base + CR);
	do {
		schedule();
	} while (readl_be(dev->base + CR) & which);
	Dprintk("okay!\n");
}
...
static int ns83820_stop(struct net_device *ndev)
{
	...
	/* disable interrupts */
	writel_be(0, dev->base + IMR);
	writel_be(0, dev->base + IER);
	readl_be(dev->base + IER);
...
static int ns83820_open(struct net_device *ndev)
{
	...
	writel_be(0, dev->base + TXDP_HI);
	writel_be(desc, dev->base + TXDP);
...
static void ns83820_getmac(struct ns83820 *dev, u8 *mac)
{
	...
	for (i=0; i<3; i++) {
		 ...
		 */
		writel_be(i*2, dev->base + RFCR);
		data = readl_be(dev->base + RFDR);
...
static void ns83820_set_multicast(struct net_device *ndev)
{
	...
	spin_lock_irq(&dev->misc_lock);
	val = (readl_be(rfcr) & and_mask) | or_mask;
	/* Ramit : RFCR Write Fix doc says RFEN must be 0 modify other bits */
	writel_be(val & ~RFCR_RFEN, rfcr);
	writel_be(val, rfcr);
	spin_unlock_irq(&dev->misc_lock);
}
...
static void ns83820_run_bist(struct net_device *ndev, const char *name, u32 enable, u32 done, u32 fail)
{
	...
	writel_be(enable, dev->base + PTSCR);
	for (;;) {
		loops++;
		status = readl_be(dev->base + PTSCR);
		if (!(status & enable))
			break;
		if (status & done)
			break;
...
static void ns83820_mii_write_bit(struct ns83820 *dev, int bit)
{
	...
	writel_be(dev->MEAR_cache, dev->base + MEAR);
	readl_be(dev->base + MEAR);
...
static int ns83820_mii_read_bit(struct ns83820 *dev)
{
	...
	/* drive MDC high causing the data bit to be latched */
	bit = (readl_be(dev->base + MEAR) & MEAR_MDIO) ? 1 : 0;
	dev->MEAR_cache |= MEAR_MDC;
	writel_be(dev->MEAR_cache, dev->base + MEAR);
...
static int __devinit ns83820_init_one(struct of_device *of_dev, const struct of_device_id *match)
{
	struct net_device *ndev;
	struct ns83820 *dev;
	...
	SET_NETDEV_DEV(ndev, &of_dev->dev);

	INIT_WORK(&dev->tq_refill, queue_refill);
	tasklet_init(&dev->rx_tasklet, rx_action, (unsigned long)ndev);
        
        irq = irq_of_parse_and_map(np, 0);
        of_address_to_resource(np, 0, &addr);
        
        if (!request_mem_region(addr.start, PAGE_SIZE, np->name)) {
            printk(KERN_ERR "%s: could not allocate device region\n", np->full_name);
            goto out_disable;
        }
        
        if (!(dev->base = ioremap(addr.start, PAGE_SIZE))) {
            printk(KERN_ERR "%s: ioremap failed\n", np->full_name);
            goto out_disable;
        }

        dev->tx_descs = dma_alloc_coherent(NULL, 4 * DESC_SIZE * NR_TX_DESC, &dev->tx_phy_descs, GFP_KERNEL);
	dev->rx_info.descs = dma_alloc_coherent(NULL, 4 * DESC_SIZE * NR_RX_DESC, &dev->rx_info.phy_descs, GFP_KERNEL);
	err = -ENOMEM;
	...
	dev->IMR_cache = 0;
        ...
        err = request_irq(irq, ns83820_irq, IRQF_DISABLED, DRV_NAME, ndev);
	if (err) {
            dev_info(&of_dev->dev, "unable to register irq %d, err %d\n", irq, err);
            goto out_disable;
	}
	...
	if (err < 0) {
		dev_info(&of_dev->dev, "unable to get netdev name: %d\n", err);
		goto out_free_irq;
	}

	printk("%s: ns83820.c: 0x22c: %08x\n",
               ndev->name, be32_to_cpu(readl_be(dev->base + 0x22c)));
	...
	writel_be(PTSCR_RBIST_RST, dev->base + PTSCR);
	...
	dev->CFG_cache = readl_be(dev->base + CFG);
	...
	if ((dev->CFG_cache & CFG_PCI64_DET)) {
        ...
        if (dev->CFG_cache & CFG_EXT_HWHDR)
            ndev->hard_header_len  += EXT_HWHDR_SIZE;
           
	dev->CFG_cache &= (CFG_TBI_EN  | CFG_MRM_DIS   | CFG_MWI_DIS |
			   CFG_T64ADDR | CFG_DATA64_EN | CFG_EXT_125 |
			   CFG_M64ADDR | CFG_EXT_HWHDR);
	...
	if (dev->CFG_cache & CFG_TBI_EN) {
		printk(KERN_INFO "%s: enabling optical transceiver\n",
			ndev->name);
		writel_be(readl_be(dev->base + GPIOR) | 0x3e8, dev->base + GPIOR);

		/* setup auto negotiation feature advertisement */
		writel_be(readl_be(dev->base + TANAR)
		       | TANAR_HALF_DUP | TANAR_FULL_DUP,
		       dev->base + TANAR);

		/* start auto negotiation */
		writel_be(TBICR_MR_AN_ENABLE | TBICR_MR_RESTART_AN,
		       dev->base + TBICR);
		writel_be(TBICR_MR_AN_ENABLE, dev->base + TBICR);
		dev->linkstate = LINK_AUTONEGOTIATE;

		dev->CFG_cache |= CFG_MODE_1000;
	}

	writel_be(dev->CFG_cache, dev->base + CFG);
	dprintk("CFG: %08x\n", dev->CFG_cache);

	if (reset_phy) {
		printk(KERN_INFO "%s: resetting phy\n", ndev->name);
		writel_be(dev->CFG_cache | CFG_PHY_RST, dev->base + CFG);
		msleep(10);
		writel_be(dev->CFG_cache, dev->base + CFG);
	}
	 ...
	 * the PCI layer.  FIXME.
	 */
	if (readl_be(dev->base + SRR))
		writel_be(readl_be(dev->base+0x20c) | 0xfe00, dev->base + 0x20c);
#endif
	...
	writel_be(TXCFG_CSI | TXCFG_HBI | TXCFG_ATP | TXCFG_MXDMA512
		| ((1600 / 32) * 0x100),
		dev->base + TXCFG);

	/* Flush the interrupt holdoff timer */
	writel_be(0x000, dev->base + IHR);
	writel_be(0x100, dev->base + IHR);
	writel_be(0x000, dev->base + IHR);
	...
	writel_be(RXCFG_AEP | RXCFG_ARP | RXCFG_AIRL | RXCFG_RX_FD
		| RXCFG_STRIPCRC
		//| RXCFG_ALP
...
#endif
	writel_be(VRCR_INIT_VALUE, dev->base + VRCR);
...
#define VTCR_INIT_VALUE VTCR_PPCHK
#endif
	writel_be(VTCR_INIT_VALUE, dev->base + VTCR);

	/* Ramit : Enable async and sync pause frames */
	/* writel_be(0, dev->base + PCR); */
	writel_be((PCR_PS_MCAST | PCR_PS_DA | PCR_PSEN | PCR_FFLO_4K |
		PCR_FFHI_8K | PCR_STLO_4 | PCR_STHI_8 | PCR_PAUSE_CNT),
		dev->base + PCR);

	/* Disable Wake On Lan */
	writel_be(0, dev->base + WCSR);

	ns83820_getmac(dev, ndev->dev_addr);

	/* Yes, we support dumb IP checksum on transmit */
        if ((dev->CFG_cache & CFG_EXT_HWHDR) == 0)
	...
	printk(KERN_INFO "%s: ns83820 v" VERSION ": DP83820 v%u.%u: %02x:%02x:%02x:%02x:%02x:%02x io=0x%08llx irq=%d f=%s\n",
               ndev->name,
               (unsigned)readl_be(dev->base + SRR) >> 8,
               (unsigned)readl_be(dev->base + SRR) & 0xff,
...
out_cleanup:
	writel_be(0, dev->base + IMR);	/* paranoia */
	writel_be(0, dev->base + IER);
	readl_be(dev->base + IER);
out_free_irq:
	rtnl_unlock();
...
out_disable:
	if (dev->base)
		iounmap(dev->base);
	dma_free_coherent(NULL, 4 * DESC_SIZE * NR_TX_DESC, dev->tx_descs, dev->tx_phy_descs);
	dma_free_coherent(NULL, 4 * DESC_SIZE * NR_RX_DESC, dev->rx_info.descs, dev->rx_info.phy_descs);
	free_netdev(ndev);
...
static void __devexit ns83820_remove_one(struct of_device *of_dev)
{
    struct net_device *ndev = dev_get_drvdata(&of_dev->dev);
    struct ns83820 *dev = PRIV(ndev); /* ok even if NULL */
    struct device_node *np = of_dev->node;
    unsigned irq = irq_of_parse_and_map(np, 0);
...
static struct of_platform_driver of_driver = {
    .name = "ns83820",
    .match_table = match,
...
static int __init ns83820_init(void)
{
	printk(KERN_INFO "ns83820.c: National Semiconductor DP83820 10/100/1000 driver.\n");
        return of_register_platform_driver(&of_driver);
}

static void __exit ns83820_exit(void)
{
        of_unregister_platform_driver(&of_driver);
